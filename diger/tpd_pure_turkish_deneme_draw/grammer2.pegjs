...
init_declarator
	= _ left:name _ exp: (dogru / yanlis / draw_statement / shape_object_with_pr / names_with_pr / math_functions / expression_statement_no_nl) _ "olsun" {
...
factor2 = "(" logical_statement ")" 
 ...
 draw_statement
 = dt:("yana_çiz" / "üste_çiz") _ sos: names_with_pr _ nl{
...
names_with_pr
 = '(' sos: (_ (draw_statement / name_expression / shape_object_with_pr) )* ')'
 {
   var items = []
...
name_expression
 = ne: ((integer "*" name) / (name "*" integer) / name){

  if(Array.isArray(ne))
  {
    if(typeof ne[0]== 'number')
        return {'type':'expression', 'repetition': ne[0], 'text':ne[2], 'lineNumber': location().start.line, 'start':location().start.column-1, 'end':location().end.column-1};    
    else (typeof ne[0]== 'string')
        return {'type':'expression', 'magnify': ne[2], 'text':ne[0], 'lineNumber': location().start.line, 'start':location().start.column-1, 'end':location().end.column-1};    
  }
  else
      return {'type':'expression', 'repetition': 1, 'text':text(), 'lineNumber': location().start.line, 'start':location().start.column-1, 'end':location().end.column-1};    
}
